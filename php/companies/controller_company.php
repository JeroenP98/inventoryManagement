...
if(isset($_GET["action"])){
  ...
  if($action === "add"){
    ...
    UserController::editCompany();
  } elseif($action === "delete"){
    UserController::deleteCompany();
  }

} else {
  header("Location: ../companies/GUI_companies.php");
  exit;
};
...
class UserController {
  ...
  public static function addCompany(){
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      if ( empty($name) || empty($street) || empty($zipcode) || empty($city) || empty($country_code) || empty($email_adress) || empty($phone_number)) {
      ...
      try { //try to excecute sql query, or display error message
        ...
        $sql = "INSERT INTO `companies`(`name`, `street`, `house_nr`, `zipcode`, `city`, `country_code`, `email_adress`, `phone_number`) VALUES ('$name', '$street', '$house_nr', '$zipcode', '$city', '$country_code', '$email_adress', '$phone_number')";
        ...
        $action = "add";
        $object_type = "Company";
        LogfileHandler::addLogfileRecord($action, $object_type, $name, "new Company");
      ...
      header("location: GUI_companies.php?action=add&status=succes&company=$name");
      exit;
    }
  }
  ...
  public static function editCompany(){
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
      ...
      try {  
        ...
        $sql = "UPDATE `companies` SET `name`='$name',`street`='$street',`house_nr`='$house_nr',`zipcode`='$zipcode',`city`='$city',`country_code`='$country_code',`email_adress`='$email_adress',`phone_number`='$phone_number' WHERE `id` = $id";
        ...
        $action = "edit";
        $object_type = "Company";
        LogfileHandler::addLogfileRecord($action, $object_type, $name, "edit Company");
      ...
      header("location: GUI_companies.php?action=edit&status=succes&company=$name");
      exit;
  ...
  public static function deleteCompany(){
    ...
    $action = "delete";
    $object_type = "company";
    LogfileHandler::addLogfileRecord($action, $object_type, $name, "delete company");

    // return back to article overview
    header("location: GUI_companies.php?action=delete&status=succes&company=$name");
    exit;
