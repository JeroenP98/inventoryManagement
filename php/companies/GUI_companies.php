...
<body>
  ...
  <!-- start new Company Modal -->
  <div class="modal fade" id="newCompanyModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h1 class="modal-title fs-5" id="exampleModalLabel">Create a new Company</h1>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <form method="POST" action="controller_company.php?action=add">
            <div class="row mb-3">
  ...
  <div class="container">
    ...
    <table class="table table-striped table-sm" id="table">
      ...
      $sql_count = "SELECT COUNT(*) AS count FROM companies";
      ...
      $sql = "SELECT `id`, `name`, CONCAT(street, ' ', house_nr) AS 'address', `zipcode`, `city`, `country_code`, `email_adress`, `phone_number` FROM `companies`
      ...
      while($row = $result->fetch_assoc()) {
        ...
        <td>
        <a class='btn btn-primary' href='GUI_companyEditor.php?id=$row[id]'>Edit</a>
        <a class='btn btn-danger' href='controller_company.php?action=delete&id=$row[id]'>Delete</a>
        </td>
        </tr>";
