...
<body>
  ...
  <div class="container">
    ...
    <table class="table table-striped table-sm" id="table">
      ...
      $sql = "SELECT `id`, `name`, CONCAT(street, ' ', house_nr) AS 'address', `zip_code`, `city`, `country_code`, `email_adress`, `phone_number` FROM `functions`
      ...
      while($row = $result->fetch_assoc()) {
        ...
        <td>$row[can_access_orders]</td>
        <td>$row[can_access_relations]</td>
        <td>$row[can_access_articles]</td>
        <td>$row[can_access_employees]</td>

        <td>
        <a class='btn btn-primary' href='GUI_functionEditor.php?id=$row[id]'>Edit</a>
        <a class='btn btn-danger' href='controller_functions.php?action=delete&id=$row[id]'>Delete</a>
        </td>
        </tr>";
