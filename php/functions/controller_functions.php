...
if(isset($_GET["action"])){
  $action = $_GET["action"];
  if($action === "add"){
    UserController::addFunction();
  } elseif($action === "edit"){
    UserController::editFunction();
  } elseif($action === "delete"){
    UserController::deleteFunction();
  }

} else {
  header("Location: ../functions/GUI_functions.php");
  exit;
};
...
class UserController {
  ...
  public static function addFunction(){
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      try { //try to excecute sql query, or display error message
        ...
        $sql = "INSERT INTO `functions`(`name`, `street`, `house_nr`, `zip_code`, `city`, `country_code`, `email_adress`, `phone_number`) VALUES ('$name', '$street', '$house_nr', '$zip_code', '$city', '$country_code', '$email_adress', '$phone_number')";
      ...
      header("location: GUI_functions.php?action=add&status=succes&function=$name");
      exit;
    }
  }
  ...
  public static function editFunction(){
    ...
    if($_SERVER['REQUEST_METHOD'] == 'POST'){
      ...
      try {  
        ...
        $sql = "UPDATE `functions` SET `name`='$name',`street`='$street',`house_nr`='$house_nr',`zip_code`='$zip_code',`city`='$city',`country_code`='$country_code',`email_adress`='$email_adress',`phone_number`='$phone_number' WHERE `id` = $id";
        ...
        $action = "edit";
        $object_type = "Function";
        LogfileHandler::addLogfileRecord($action, $object_type, $name, "edit Function");
      ...
      header("location: GUI_functions.php?action=edit&status=succes&function=$name");
      exit;
  ...
  public static function deleteFunction(){
    ...
    $stmt = "SELECT name FROM functions WHERE id=$id";
    ...
    $action = "delete";
    $object_type = "company";
    LogfileHandler::addLogfileRecord($action, $object_type, $name, "delete Function");

    // return back to article overview
    header("location: GUI_functions.php?action=delete&status=succes&function=$name");
    exit;
